...
async function printReady(){
  ...
  await Promise.all(images.map(async img=>{
    ...
    if(!img.complete){await new Promise(resolve=>{img.addEventListener('load',resolve,{once:true});img.addEventListener('error',resolve,{once:true});setTimeout(resolve,1600)})}
  }));
...
function printCleanup(){
  window.setTimeout(()=>{const area=$('printArea');if(area)area.innerHTML='';},250);
}
...
function printAttendance(cls,date,records,teacher){
  ...
  const groups=Math.max(1,Math.min(5,Math.ceil(students.length/50)));
...
function bindEvents(){
  ...
  $('modalRoot').addEventListener('click',e=>{
    if(e.target===e.currentTarget||e.target.matches('[data-close-modal]')||e.target.closest('[data-modal-close]'))closeModal();
  });
  ...
  document.addEventListener('submit',e=>{
    ...
    window.setTimeout(()=>{form.dataset.submitting='0';if(submit)submit.disabled=false},1800);
  },true);
