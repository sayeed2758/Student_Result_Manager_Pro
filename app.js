...
function saveCurrentExam(){const e=currentExam();if(!e){toast('Create or select an exam first.');return}const total=Number($('totalMarks').value),date=$('examDate').value;if(!Number.isInteger(total)||total<1){toast('Enter valid Total Marks.');return}e.totalMarks=total;e.date=date||e.date||todayISO();ensureExamStudents(e);Object.keys(e.marks).forEach(id=>{if(e.statuses[id]==='absent'){e.marks[id]='';return}if(e.marks[id]!==''){const n=Number(e.marks[id]);if(!Number.isFinite(n)||n<0||n>total){e.marks[id]='';e.statuses[id]='pending'}}});persist();renderAll();toast('Exam saved successfully.')}
...
function updateExam(exam,name,totalMarks,date){const clean=String(name||'').trim(),total=Number(totalMarks);if(!clean||!Number.isInteger(total)||total<1){toast('Enter valid exam details.');return false}if(classData().exams.some(e=>e.id!==exam.id&&e.name.toLowerCase()===clean.toLowerCase()&&e.date===date)){toast('Another exam has the same name and date.');return false}exam.name=clean;exam.totalMarks=total;exam.date=date||exam.date||todayISO();ensureExamStudents(exam);Object.keys(exam.marks).forEach(id=>{if(exam.statuses[id]==='absent'){exam.marks[id]='';return}if(exam.marks[id]!==''){const n=Number(exam.marks[id]);if(!Number.isFinite(n)||n<0||n>total){exam.marks[id]='';exam.statuses[id]='pending'}}});persist();renderAll();toast('Exam updated.');return true}
...
async function printResult(exam=currentExam()){
  ...
  if(logo && !logo.complete){
    await new Promise(resolve=>{logo.addEventListener('load',resolve,{once:true});logo.addEventListener('error',resolve,{once:true});setTimeout(resolve,1500);});
  }
  window.print();
}
...
function printStudentReport(studentId){
  ...
  if(logo&&!logo.complete)logo.addEventListener('load',go,{once:true}),logo.addEventListener('error',go,{once:true}),setTimeout(go,1200);else setTimeout(go,50);
}
