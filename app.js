...
async function preparePrintAreaAndPrint(){
  const area=$('printArea');
  if(!area)return;
  const images=[...area.querySelectorAll('img')];
  await Promise.all(images.map(async img=>{try{if(img.decode)await img.decode()}catch(_){ }if(!img.complete){await new Promise(resolve=>{img.addEventListener('load',resolve,{once:true});img.addEventListener('error',resolve,{once:true});setTimeout(resolve,1800)})}}));
