...
const valid = Number.isFinite(value) && value >= 0 && value <= e.totalMarks;
  ...
  async function installApp() {
    ...
    if (isStandalone) {
      toast('Student Result Manager is already installed.');
      return;
    }
    ...
    if (!state.deferredPrompt) {
      ...
      openModal(`
        <div class="modal-head">
          <div>
            <span class="eyebrow">PWA INSTALL</span>
            <h3>Install Student Result Manager</h3>
            <p class="profile-subtitle">
              Chrome controls when the native install prompt is available.
            </p>
          </div>
          <button class="close-modal" type="button" data-modal-close>×</button>
        </div>
        <div class="tip">
          <strong>${secure ? 'The app is ready for installation.' : 'Open the website over HTTPS first.'}</strong>
          <p style="margin:7px 0 0">
            If Chrome has not supplied the native prompt yet, open the Chrome menu (⋮) and choose
            <b>Install app</b> or <b>Add to Home screen</b>. Do not choose only “Create shortcut”.
          </p>
        </div>
        <div class="modal-actions">
    ...
    try {
      ...
      toast('Chrome could not open the install prompt. Try Chrome menu → Install app.');
    }
  }
